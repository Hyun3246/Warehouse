...
def KNearestClassify(training, testSet, label, k):
    """k-최근접 이웃 실행 + true positives, false positives, true negatives, and false negatives 반환"""
...
import sklearn.linear_model         # 로지스틱 회귀
...
def buildROC(trainingSet, testSet, title, plot = True):
    '''ROC 곡선 나타내기(specificity와 sensitivity의 관계 표시)'''
    model = buildModel(trainingSet, True)
    xVals, yVals = [], []
    p = 0.0
    while p <= 1.0:
